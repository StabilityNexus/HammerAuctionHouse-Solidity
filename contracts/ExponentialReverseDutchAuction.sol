 ...
 * @notice Reverse Dutch auction with exponential decay pricing.
 */
contract ExponentialReverseDutchAuction is Auction, Ownable, Pausable {
    ...
    uint256[61] private decayLookup = [
        1000000000000000000,500000000000000000,250000000000000000,
        125000000000000000,62500000000000000,31250000000000000,
        15625000000000000,7812500000000000,3906250000000000,
        1953125000000000,976562500000000,488281250000000,
        244140625000000,122070312500000,61035156250000,
        30517578125000,15258789062500,7629394531250,
        3814697265625,1907348632812,953674316406,
        476837158203,238418579102,119209289551,
        59604644775,29802322388,14901161194,
        7450580597,3725290298,1862645149,
        931322574,465661287,232830643,
        116415322,58207661,29103831,
        14551915,7275958,3637979,
        1818989,909495,454747,
        227373,113687,56843,
        28422,14211,7105,
        3553,1776,888,
        444,222,111,
    ...
    function getDecayValue(uint256 x) internal view returns (uint256) {
        ...
        return higherValue - ((higherValue - lowerValue) * remainder) / 1e5;
    }
        ...
        returns (uint256)
    {
        ...
        uint256 timeElapsed =
            block.timestamp - (auction.deadline - auction.duration);

        uint256 x = timeElapsed * auction.decayFactor;
        uint256 decayValue = getDecayValue(x);

        return auction.minPrice +
            ((auction.startingPrice - auction.minPrice) * decayValue) / 1e18;
    }
